...
namespace MessengerData
{
    public class ApplicationDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<MessageComment>().HasOne(x => x.Message).WithMany().HasForeignKey(x => new {x.MessageDate, x.MessageGuid}).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<MessageComment>().HasOne(x => x.CommentedMessage).WithMany().HasForeignKey(x => new { x.CommentedMessageDate, x.CommentedMessageGuid}).OnDelete(DeleteBehavior.ClientCascade);
            ...
            modelBuilder.Entity<DeletedMessage>().HasOne(x => x.User).WithMany().HasForeignKey(x => x.UserGuid).OnDelete(DeleteBehavior.ClientCascade);
